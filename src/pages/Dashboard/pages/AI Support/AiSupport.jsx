...
// Deduplicate data
const invoiceFlowData = rawInvoiceFlowData.reduce((acc, step) => {
    const existing = acc.find((s) => s.id === step.id && s.label === step.label);
    if (!existing) {
        acc.push({ ...step });
    } else {
        existing.descriptions = [...new Set([...existing.descriptions, ...step.descriptions])];
        existing.hasLoop = existing.hasLoop || step.hasLoop || false;
        existing.loopConnections = existing.loopConnections || step.loopConnections;
        existing.isBottleneck = existing.isBottleneck || step.isBottleneck || false;
...
const CustomNode = ({ data }) => {
    ...
    const statusColor =
        extra.isDropout ? "bg-red-600" :
            extra.isBottleneck ? "bg-yellow-600" :
                extra.hasLoop ? "bg-purple-600" : "bg-blue-600";
    return (
        <div className={`min-w-64 min-h-5 text-gray-200 flex justify-between hover:cursor-pointer z-20 border ${statusColor}`}>
...
const buildEdges = (steps, prefix = "") => {
    ...
    for (let i = 0; i < steps.length - 1; i++) {
        const color =
            steps[i].isDropout || steps[i + 1].isDropout ? "#dc2626" :
                steps[i].isBottleneck || steps[i + 1].isBottleneck ? "#eab308" : "#6b7280";
        edges.push({
            id: `${prefix}e${steps[i].id}-${steps[i].label}-${steps[i + 1].id}-${steps[i + 1].label}`,
    ...
    steps.forEach((step) => {
        if (step.hasLoop && step.loopConnections) {
            const sourceLabel = steps.find(s => s.id === step.loopConnections.from)?.label || "unknown";
            const targetLabel = steps.find(s => s.id === step.loopConnections.to)?.label || "unknown";
            edges.push({
    ...
    return (
        <div className="flex w-full h-full relative">
            ...
            <div className="flex flex-1 flex-col md:flex-row gap-4 p-4">
                ...
                {/* AI Chat Sidebar */}
                <div className="w-80 border rounded bg-gray-800 p-4 flex flex-col">
                    ...
                    >
                        {chatMessages.map((msg, index) => (
                            <div
                                key={index}
                                className={`mb-2 p-2 rounded max-w-[80%] w-fit ${msg.sender === "user"
