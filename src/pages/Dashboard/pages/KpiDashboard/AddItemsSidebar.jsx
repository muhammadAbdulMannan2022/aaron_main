...
const AddWidgetSidebar = ({ isOpen, onClose, onAdd, onToggle, isopen }) => {
  ...
  const { data: kpiList, isLoading: kpiListLoading } =
  ...
  return (
    <div
      className={`w-80 h-full z-[9999] shadow-md border-l transform transition-transform duration-200 ease-in-out ${
        isOpen ? "translate-x-0" : "translate-x-full"
      } fixed right-0 z-50 h-auto min-h-0 sidebar`}
      style={{
    ...
    >
      <div className="flex flex-col min-h-0 relative">
        <div
          onClick={onToggle}
          className="absolute w-10 items-center justify-center -left-10 -top-10 px-4 py-2 rounded-s-full cursor-pointer hover:opacity-80 transition-opacity"
          style={{
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full px-3 py-3 border rounded-lg focus:outline-none focus:ring-2 transition-all appearance-none hover:cursor-pointer custom-select"
                  style={{
                ...
                >
                  ...
                  {kpiList &&
                    kpiList.map((kpi) => (
                      <option key={kpi.id} value={kpi.kpi_name}>
                        {kpi.kpi_name}
                      </option>
                    ))}
                </select>
                ...
                return (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-all hover:opacity-80 ${
                      selectedWidget === widget.id ? "ring-2" : ""
                    }`}
                    style={{
                      backgroundColor: "var(--color-gray-button-bg)",
                      borderColor: "var(--color-gray-button-bg)",
