...
export default function ProcessVariantsModal({
  onClose,
  selectedVarients,
  setSelectedVarients: setToManState,
  data,
}) {
  const variants = data || [];
  ...
  return (
    <div className="p-4 w-full md:w-md max-w-md">
        ...
        {isExpanded && (
          <div className="mt-2 border border-[var(--color-button-outline)] rounded-lg bg-[var(--color-main-bg)] max-h-64 overflow-y-auto">
            {variants.map((variant, index) => (
              <div
                key={variant.id}
                className={`flex items-center justify-between p-4 hover:bg-[var(--color-gray-button-bg)] transition-colors cursor-pointer ${
                  variant && index !== variants.length - 1
                    ? "border-b border-[var(--color-gray-button-bg)]"
                    : ""
                }`}
              ...
              >
                ...
                <span className="text-[var(--color-dark-text)] text-sm">
                  {variant.case_count}
                </span>
              </div>
            ))}
      ...
      {selectedCount > 0 && (
        <div className="mb-6">
          <h3 className="text-[var(--color-text-primary)] text-sm font-medium mb-3">
            Selected Variants:
          </h3>
      ...
      {/* Selection Info */}
      {selectedCount > 0 && (
        <div className="mb-4 text-sm text-[var(--color-dark-text)]">
          {selectedCount}/5 variants selected
        </div>
      )}
